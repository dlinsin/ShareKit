...
@implementation SHKyfrog

@synthesize yfrogAPIKey;
...
- (id)init {
	...
	if (self) {	
        ...
        self.yfrogAPIKey = SHKyfrogAPIKey;
	}	
	return self;
}
...
- (void)tokenAccessModifyRequest:(OAMutableURLRequest *)oRequest {	
	NSDictionary *formValues = [super.pendingForm formValues];
...
- (void)show {
	[super.item setCustomValue:super.item.title forKey:@"status"];
	[self showTwitterForm];
}

- (void)showTwitterForm {
	SHKyfrogForm *rootView = [[SHKyfrogForm alloc] initWithNibName:nil bundle:nil];	
	rootView.delegate = self;
    
	// force view to load so we can set textView text
	[rootView view];
    
	rootView.textView.text = [super.item customValueForKey:@"status"];
...
- (void)sendForm:(SHKyfrogForm *)form {	
	[super.item setCustomValue:form.textView.text forKey:@"status"];
	[self tryToSend];
}
...
- (BOOL)validate {
	NSString *status = [super.item customValueForKey:@"status"];
...
- (NSData*)imageData {
	...
	NSData *imageData = UIImageJPEGRepresentation([super.item image], compression);
	...
	while ([imageData length] > 700000 && compression > 0.1) {
		...
		compression -= 0.1;
		imageData = UIImageJPEGRepresentation([super.item image], compression);
